...
spdlog_python_install() {
    PREVIOUS_PWD=$PWD
    TMP=`mktemp -d`
    echo ">>>>> installing spdlog package"
...
apparmor_monkeys_install() {
    PREVIOUS_PWD=$PWD
    TMP=`mktemp -d`
    echo ">>>>> installing apparmor_monkeys package"
...
pycapnp_install() {
    echo ">>>>> installing pycapnp package"
...
butter_install() {
    ...
    PREVIOUS_PWD=$PWD
    TMP=`mktemp -d`
    echo ">>>>> installing butter package"
...
rpyc_install() {
    ...
    cd $PREVIOUS_PWD
    rm -rf $TMP
    echo ">>>>> pip3 installed rpyc package"
}

py_lmdb_install() {
    PREVIOUS_PWD=$PWD
    TMP=`mktemp -d`
    echo ">>>>> installing lmdb package"
    ...
    cd $PREVIOUS_PWD
    rm -rf $TMP
    echo ">>>>> pip3 installed lmdb package"
}

# Install other required packages
# Utilizing distribution installed psutil (5.5.1) and cryptography versions
pip3_3rd_party_installs(){
    echo ">>>>> installing pip3 packages"
    start=`date +%s`
    pip3 install 'redis==4.3.4' 'hiredis==2.0.0' 'netifaces==0.11.0' --verbose
    pip3 install 'cgroups==0.1.0' 'cgroupspy==0.2.2' --verbose
    pip3 install 'fabric3==1.14.post1' 'pyserial==3.5' --verbose
    pip3 install 'pycryptodomex==3.15.0' pyyaml==6.0.1 --verbose
