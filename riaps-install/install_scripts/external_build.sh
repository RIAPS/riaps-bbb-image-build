...
build_opendht() {
    ...
    git checkout 2.1.10
    ./autogen.sh
    ./configure PKG_CONFIG_PATH=/usr/local/pkgconfig MsgPack_LIBS="-L/usr/lib/arm-linux-gnueabihf -lmsgpackc" MsgPack_CFLAGS=-I/usr/include/arm-linux-gnueabihf CFLAGS=-I/tmp/3rdparty/opendht/argon2/include
    make -j2
    sudo make install
    cd $PREVIOUS_PWD
    sudo rm -rf $TMP
